...
2.) This code must be spawned with an adequate stack size.
    ...
    -> taskSpawn ("pMount", 20, 0, 20000, dosPartMountAta,0,0)
    ...
    -> taskSpawn ("pShow", 20, 0, 20000, dosPartShowAta,0,0)



    Do not call it directly from the shell ala:

    -> dosPartMountAta 0,0 -- WRONG!, may overflow default stack.
    -> dosPartShowAta 0,0  -- WRONG!, may overflow default stack.
...
Standard vxWorks makes are expected.  Should work with any arch.
...
SOME ataDrv() INFORMATION:
--------------------------

The VxWorks ataDrv() drivers ATA parameters in (setup in the 
BSP file $WIND_BASE/target/config/pc486/pc.h) for ATA0_CONFIG 
and ATA1_CONFIG must be setup appropriately for the particular
system.

At the ataDrv driver level:

If ATA_GEO_CURRENT is used, ataDrv() uses current drives
(PCHS) geometry parameters, if possible.

If ATA_GEO_PHYSICAL is used, the ata driver uses the physical
geometry parameters stored on the drive, if possible.  

If ATA_GEO_FORCE is used, the ata driver uses values stored in 
the "ataTypes[][]" table in sysLib.c. The geometry parameters
are the number of sectors per track, the number of heads, and
the number of cylinders. The ataTypes[][] table has two entries:
the first is for drive 0, the second is for drive 1. The table 
has two other members which are used by the driver. They are the
number of bytes per sector and the write pre-compensation cylinder.
These two members should also be set to properly match the BIOS/OS
setting used.  These settings will be attempted to be translated
by dosPartRdLChsAta().

For the PC x86 BSP's ATA_GEO_CURRENT is the default for VxWorks
5.3.1 FCS.  ATA_GEO_CURRENT is the recommended setting for newer
drives and BIOS's and when using this code.  Else ataTypes must
precisely match what the other BIOS/OS/Controller uses for PCHS.


By default, with INCLUDE_ATA, vxWorks is setup for one ATA device
on the first controller.  The others are disabled to prevent timeouts.
If you have more than one hard disk (two per controller max.) or more
than one controller (two max) then you must modify the ataResources
struct array in sysLib.c to support more drives.

For example, to support two controllers, each with two drives:
(4 drives total)  I defined ataResources as follows:
...
ATA_RESOURCE ataResources[ATA_MAX_CTRLS] =
    {
    {
     ...
     IDE_LOCAL, ATA0_NUM_DRIVES, ATA0_INT_VEC, ATA0_INT_LVL, ATA0_CONFIG,
    ...
    },
    {
     ...
     IDE_LOCAL, ATA1_NUM_DRIVES, ATA1_INT_VEC, ATA1_INT_LVL, ATA1_CONFIG,
     ATA_SEM_TIMEOUT, ATA_WDG_TIMEOUT, 0, 0
    }
    };
...
ATA0_NUM_DRIVES and ATA1_NUM_DRIVES are defined as 2.
ATA0_INT_LVL is defined as 0x0e.
ATA1_INT_LVL is defined as 0x0f.

And all other macros are defined per the default values as 
shipped with pc386/pc486 BSP's.
...
#include "drv/hdisk/ataDrv.h"   /* ATA driver header (vxWorks)      */
#include "dosPartLibAta.h"      /* This libraries header file.      */


/* imports */

IMPORT ATA_CTRL ataCtrl [ATA_MAX_CTRLS];                 /* ataDrv.h */
IMPORT ATA_TYPE ataTypes [ATA_MAX_CTRLS][ATA_MAX_DRIVES];/* ataDrv.h */ 
IMPORT BOOL ataDrvInstalled;                             /* ataDrv.h */
...
LOCAL int extPartLevel = 0; /* static extended partitions counting  */

/* globals */

/* local function declarations */

STATUS dosPartParseAta  /* fills list with valid ata parts  */
    (
...
STATUS dosPartRdLbaAta  /* read ataSec given dos CHS values */
    (
    BOOL ctrl,          /* 0:primary ctrl, 1:secondary ctrl */
    BOOL drive,         /* (0: Master/Single or 1: Slave.)  */
    ULONG lbaSec,       /* lba sector to read data from.    */
    UCHAR *ataSecBuf    /* buffer for the read sector data  */
    );

/*****************************************************************************
*
* dosPartMountAta - mounts all partitions on a HDD.
*
* This routine mounts all MSDOS file systems residing in partitions
* created by FDISK.exe for given ATA physical HDD device.
...
* calls dosPartParseAta() passing a pointer to the new list.  When
* dosPartParseAta returns, it has filled the list with any partition
...
* are created using a drivers block device creation routine, for ATA it
* is ataDevCreate).  We then attempt to mount each of the blockDevices
* with dosFsDevInit(). Finally, we clean up with lstFree() & free().
* 
* This code must be spawned with a adequate stack size since 
* dosPartParseAta() is recursive (recursion uses stack)
...
* -> sp dosPartMountAta,0,0 
*    or
* -> taskSpawn ("pMount", 20, 0, 30000, dosPartMountAta,0,0)
*
...
STATUS dosPartMountAta      /* mounts partitions on drive/ctrl  */
    (
    BOOL ctrl,              /* 0: Master/Single or 1: Slave.    */
    BOOL drive              /* 0:primary ctrl, 1:secondary ctrl */
    )
    ...
    PART_NODE * pPartNode;  /* ptr to dll of valid partitions   */
                            /* see dosPartLibAta.h for typedef  */
    BLK_DEV *   pAtaBlkDev; /* BLK_DEV ptr for ata device.      */
    ...
    if ((pVxPartDll = (LIST *) calloc ((sizeof (LIST)), 1)) == NULL)
        {
        printErr ("dosPartMountAta error. calloc failed.\n");
        return (ERROR);         
        } 
    ...
    if  ((dosPartParseAta (ctrl, drive, pVxPartDll)) == ERROR)
        {
        printErr ("dosPartMountAta error. " 
                  "dosPartParseAta returned error.\n");
        return (ERROR);
        }
    ...
    if ((pPartNode = (PART_NODE *) lstFirst (pVxPartDll))
         == NULL)
        {
        printErr ("dosPartMountAta error: lstFirst returned NULL.\n");
        return (ERROR);
        }
    ...
    for (i=0; i < pVxPartDll->count; i++)
        ...
        /* Create a unique device name for file system partitions */

        sprintf (devName, "/ataCtrl%dDrive%dPart%d", ctrl, drive, i);


        /* Create a block device for the partition */

        if ((pAtaBlkDev = ataDevCreate (ctrl, drive, 
             pPartNode->dosPartNumBlks, 
             pPartNode->dosPartBlkOffset)) 
             == (BLK_DEV *)NULL)
            {
            printErr ("dosPartMountAta error. " 
                      "error during ataDevCreate.\n");
            return (ERROR);         
            }
        ...
        printf ("Mounting partition %02d - VxWorks device Name \"", i);
        printf (devName);
        printf ("\"\n");
        ...
        printf ("Approximate Partition size: %ld MBs\n\n",
                 pPartNode->dosPartNumBlks * BYTES_PER_SECTOR / 0x00100000);

        if (dosFsDevInit(devName, pAtaBlkDev, NULL) == 
            (DOS_VOL_DESC *) NULL)
            {
            ...
            printErr ("Error occurred attempting to mount partition %02d.\n",
                        i);
            ...
            printErr ("Possible dosPartMountAta has already "
...
/*****************************************************************************
*
* dosPartParseAta - parse ata partitions on given drive/ctrl.
*
...
* a (recursive) call to dosPartParseAta is made to parse the extended
...
* so be aware.
* 
* Any active boot partition (dospt_status == 0x80, there should
* only be one on a drive.) is placed first on the list, while all
* other partitions containing "valid" file systems are placed at 
* the end of the list in the order they are found.  When all found 
* partition tables have been parsed, OK status is returned.
* 
* RETURNS: OK or ERROR.
*
*/
...
STATUS dosPartParseAta      /* fills list with valid ata parts  */
    ...
    {
    UCHAR * secBuf;             /* a buffer for ata sector data */
    PART_NODE * pPartNode;      /* dll ptr of valid partitions  */
                                /* see dosPartLibAta.h typedef  */
    ...
    /* allocate a local buffer for the read sectors MBR/Part data */

    if ((secBuf = malloc (BYTES_PER_SECTOR)) == NULL)
        {
        printErr ("dosPartParseAta error. " 
                  "Error during malloc'ing sector buffer.\n");
        return (ERROR);
        }


    /* read the ata partition sector (LBA format) into a buffer */

    if ((dosPartRdLbaAta (ctrl, drive, currentOffset, secBuf)) == ERROR)
        {
        printErr ("dosPartParseAta error. " 
                  "dosPartRdLbaAta returned error.\n");
        return (ERROR);
        }
    ...
    if ((secBuf[PART_SIG_ADRS] != PART_SIG_MSB) || 
        ...
        {
        printErr ("dosPartParseAta error. " 
                  "Sector buffer contains an invalid"
	...
	partOffset = i * (sizeof (DOS_PART_TBL));
        ...
        if ((secBuf[DOS_BOOT_PART_TBL + partOffset + SYSTYPE_OFFSET]
           ...
           if (extPartLevel > 0)
            ...
            extPartLevel++;
            dosPartParseAta (ctrl, drive, pVxPartDll);
            extPartLevel--;
            }
        /* Increment to next partition (0x1be-0x1ce-0x1de-0x1ee) */
        }
...
/*****************************************************************************
*
* dosPartRdLbaAta - reads a LBA sector from a ATA device. 
*
* This routine is not intended to be user callable.
* 
* A device dependent sector read routine.  This routine is passed
* a pointer to a buffer for the sector data.  This routine is also
* passed in LBA format the absolute sector value to read.  This code
* will convert the LBA to ATA PCHS and call ataRawio to fill the buffer
* with the sector data requested.  If this is accomplished without error,
* it returns OK status.
* 
* The LBA is converted to a PCHS before the read per ataTypes 
* parameters. 
*
* RETURNS: OK or ERROR.
*
...
STATUS dosPartRdLbaAta  /* read ataSec given dos CHS values */
    ...
    {
    ATA_RAW   ataRaw;                           /* ataDrv.h */
    ATA_CTRL * pCtrl = &ataCtrl[ctrl];          /* ataDrv.h */
    ATA_TYPE * pType = &ataTypes[ctrl][drive];  /* ataDrv.h */
    ATA_DRIVE *pDrive = &pCtrl->drive[drive];   /* ataDrv.h */
    LOCAL USHORT tempN;         /* for N based BIOS xlation */
    LOCAL UINT16 ataCylinder;   /* PCHS Cyl for ataRawio()  */
    LOCAL UINT8 ataHead;        /* PCHS Head for ataRawio() */
    LOCAL UINT8 ataSector;      /* PCHS Sec for ataRawio()  */      


    /* ensure the ata driver is on & drive/ctrl are valid.  */

    if ((ctrl >= ATA_MAX_CTRLS) || (drive >= ATA_MAX_DRIVES) ||
         !ataDrvInstalled || !pCtrl->installed)
        {
        printErr ("dosPartRdLbaAta error:"
                  " Invalid ctrl/drive or no ataDrv present.\n");
        return (ERROR);
        }


    /* Check if drive supports LBA (newer ata drive do) */
    if (!(pDrive->okLba))
        {
		printErr ("\ndosPartRdLbaAta Warning:\n" 
                  "Drive reports not supporting LBA\n"
		  "Non-fatal issue, continuing...\n");
        }


    /* Convert the LBA to PCHS for ataRawio (from ATA-2 Spec.) */

    ataCylinder = (lbaSec / (pType->heads * pType->sectors));
    tempN = lbaSec % (pType->heads * pType->sectors);
    ataHead = (tempN / pType->sectors);
    ataSector = ((tempN % pType->sectors) + 1);


    /* setup the ataRaw structure for reading ATA PCHS sector.  */

    ataRaw.cylinder = ataCylinder;  /* cylinder to use          */
    ataRaw.head     = ataHead;      /* head to use              */
    ataRaw.sector   = ataSector;    /* sector to use            */
    ataRaw.pBuf     = ataSecBuf;    /* buffer for sector data   */
    ataRaw.nSecs    = 1;            /* One sector for a parttbl.*/
    ataRaw.direction= 0;            /* 0=read, 1=write          */


    /* read the sector, will place sector data in ataSecBuf.    */

    if ((ataRawio (ctrl, drive, &ataRaw)) == ERROR)
        {
        printErr ("dosPartRdLbaAta error:"
                   " ataRawio parameters could be invalid.\n");
        printErr ("ataRawio parameters used in ataRawio call: \n"
                  "ataCylinder = 0x%x, ataHead = 0x%x, ataSector = 0x%x\n",
                    ataCylinder, ataHead, ataSector);
...
/*****************************************************************************
*
* dosPartShowAta - parse and display ata partition data for a drive/ctrl.
*
* This routine is intended to be user callable.
*
...
* dosPartRdLbaAta() to read the sectors.  This code 
* can be removed to reduce code size by undefining: INCLUDE_PART_SHOW.
*
* RETURNS: OK or ERROR
*
*/

STATUS dosPartShowAta       /* show all partitions on ATA drive */
    (
    BOOL ctrl,              /* (0: Master/Single or 1: Slave.)  */
    BOOL drive              /* 0:primary ctrl, 1:secondary ctrl */
    )
    ...
    {
    UCHAR * secBuf;             /* a secBuf for ata sector data */
    LOCAL ULONG extPartOffset;  /* for first ext part offset    */
    LOCAL ULONG currentOffset;  /* for current ext part offset  */ 
    ...
    struct partType             /* Some partition type values & names. */
        ...
        UINT8 partTypeNum;      /* dosPartParseAta() may be modified   */
        char *partTypeName;     /* to support any other if needed      */
        } partNames[] = 
            {
    ...
    if  ((secBuf = malloc (BYTES_PER_SECTOR)) == NULL)
        {
        printErr ("dosPartShowParseAta error. " 
                  "Error during malloc'ing sector buffer.\n");
        return (ERROR);
        }
