...
DESCRIPTION -
	     myServer - Server reads from local socket and displays 
             client's message.
	     If requested, server sends a reply back to the client. Code 
             executes on VxWorks target and use VxWorks header files.

RUNNING DEMO -
	     Place the vxServer.c in your <Tornado> dir. Depending on the
